...
Button* Button::createWithSprite(std::string_view fileName, ButtonCallback activateCallback) {
    auto ret = new Button();
...
Button* Button::createWithSpriteFrameName(std::string_view frameName, ButtonCallback activateCallback) {
    auto ret = new Button();
...
Button* Button::createWithLabel(std::string_view text, std::string_view font, ButtonCallback activateCallback) {
    auto ret = new Button();
...
bool Button::initWithSprite(std::string_view fileName, ButtonCallback activateCallback) {
    if (!Button::init(std::move(activateCallback))) return false;

    m_impl->m_displayNode = CCSprite::create(fileName.data());
...
bool Button::initWithSpriteFrameName(std::string_view frameName, ButtonCallback activateCallback) {
    if (!Button::init(std::move(activateCallback))) return false;

    m_impl->m_displayNode = CCSprite::createWithSpriteFrameName(frameName.data());
    if (!m_impl->m_displayNode) return false;
...
bool Button::initWithLabel(std::string_view text, std::string_view font, ButtonCallback activateCallback) {
    if (!Button::init(std::move(activateCallback))) return false;

    m_impl->m_displayNode = CCLabelBMFont::create(text.data(), font.data());
    if (!m_impl->m_displayNode) return false;
...
void Button::registerWithTouchDispatcher() {
    ...
    // as much as i wanted to keep this header identical to geode's
    // (aside from not in v4 changes like string view and function),
    // nothing else i did worked unfortunately
    queueInMainThread([this]{
        CCTouchDispatcher::get()->setPriority(m_impl->m_touchPriority,this);
    });
}
